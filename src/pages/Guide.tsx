// Guide.tsx is the reference manual: what the dataset is, how every number in
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div id="dataset" className="mt-4 scroll-mt-24 rounded-lg border border-stone-200 bg-white p-5">
        ...
        <div className="mt-2 space-y-3 text-sm leading-relaxed text-stone-600">
          ...
          <dl className="rounded-md border border-stone-200 bg-stone-50/60 px-4 py-2 font-sans text-[13px]">
            ...
            <Row k="DMSO clusters / drug" v={<><N>{rng(nRefs)}</N> control-side contexts (median <N>{[...nRefs].sort((a, b) => a - b)[Math.floor(nRefs.length / 2)]}</N>)</>} />
            <Row k="Drug clusters / drug" v={<><N>{rng(nQs)}</N> treated subpopulations (median <N>{[...nQs].sort((a, b) => a - b)[Math.floor(nQs.length / 2)]}</N>)</>} />
            <Row k="Cluster pairs / drug" v={<><N>{rng(pairs)}</N>, one per DMSO × drug combination</>} />
      ...
      <div id="numbers" className="mt-4 scroll-mt-24 rounded-lg border border-stone-200 bg-white p-5">
        ...
        <div className="mt-2 space-y-3 text-sm leading-relaxed text-stone-600">
          ...
          <p>
            <B>Two thresholds, and they differ on purpose.</B> The <B>reference cutoff</B> is fixed at
            ...
            Rank, the two views that read the full per-drug data and can re-threshold; it spans
      ...
      <div id="colour" className="mt-4 scroll-mt-24 rounded-lg border border-stone-200 bg-white p-5">
        ...
        <div className="mt-2 space-y-3 text-sm leading-relaxed text-stone-600">
          ...
          <p>
            The unsigned ramp is low-chroma on purpose. &ldquo;Unsigned&rdquo; means the directional methods
      ...
      <div className="mt-4 grid gap-4">
        ...
        <Section id="drugs" title="Drugs: the index" href="#/">
          ...
          <p>
            ...
            {' '}(<N>1 − normalised entropy</N> of the pathway distribution, so drugs whose signal is
        ...
        <Section id="landscape" title="Landscape: everything at once" href="#/atlas">
          <p>
            ...
            {' '}{manifest.pathways.length} columns always fit, so there is no horizontal scrolling.
          </p>
          <p>
        ...
        <Section id="pathways" title="Pathways" href="#/pathways">
          ...
          <p>
            ...
            total, up, down, or <B>discordant</B> (<N>min(up, down)</N>, which finds the most
            internally-conflicted drugs). Each page links out to the MSigDB entry and into the
            Correlation view seeded with that pathway.
          </p>
        </Section>
        ...
        <Section id="correlation" title="Correlation: three modes" href="#/correlate">
          ...
          <p>
            ...
            The line <em>breaks</em> where a channel has no variance (say, a pathway never called
          ...
          <p>
            ...
            the two selected pathways outlined. That answers the second half of the question: what else is
            happening in the clusters where the pair is coupled.
          </p>
        </Section>

        <Section id="connectivity" title="Connectivity" href="#/connect">
          <p>
            Connectivity mapping in the Lamb (2006) / CMap tradition. A <B>query signature</B> is scored by
            {' '}<B>cosine similarity</B> against every one of the other {manifest.drugs.length - 1} drugs. The
            query is either any drug&rsquo;s {manifest.pathways.length}-dimensional net-direction vector, or a
            set you build by hand marking pathways up and down. Positive = <B>mimicker</B> (same program),
            negative = <B>reverser</B>, the classic repurposing read-out.
          </p>
          <p>
          ...
          <p>
            The <B>split bar</B> on each row decomposes the cosine into a <B>generic</B> component and the
            {' '}<B>specific</B> remainder. The generic part is the projection onto the atlas&rsquo;s first
            principal component, the shared proliferation-stress axis that carries roughly a quarter of all
            variance. Two drugs can score highly just by both being strongly cytotoxic; a mostly-grey bar is
            exactly that, and not evidence of shared pharmacology. Where a ChEMBL mechanism is known it is
            printed beside the hit, which is what makes class recovery visible: query Digitoxin and Ouabain
            returns first, both Na⁺/K⁺-ATPase inhibitors, with no drug metadata used in the scoring.
          </p>
        </Section>
        ...
        <Section id="consensus" title="Consensus" href="#/consensus">
          <p>
            ...
            and then removes it. <B>Panel A</B> is the consensus signature. Per pathway it gives the mean net
            direction across all {manifest.drugs.length} drugs with a 95% t-interval, plus <B>sign
            consistency</B>: among drugs that respond at all (|net| &gt; <N>{RESPONDER_MIN}</N>), the share
        ...
        <Section id="mechanism" title="Mechanism: does pathway space encode what a drug does?" href="#/mechanism">
          ...
          <p>
            <B>Where the signal does show up.</B> The per-class panel ranks each mechanism by its
            ...
            {' '}<em>never</em> the red/blue that encode up/down elsewhere.
          </p>
          <p>
            <B>Class outliers.</B> Selecting a class scores each member against the centroid of the
            {' '}<em>other</em> members. This is leave-one-out, so a single outlier cannot drag the centroid
            onto itself and hide. Two situations are separated by the <N>‖net‖</N> column (the length of
            ...
            its class, a candidate off-target or polypharmacology effect. A low score on a <B>weak</B>
          ...
          <p>
            ...
            mechanism is a single label for a drug that may hit many targets, so an incoherent class may be
            heterogeneous in reality. And a negative result here is a statement about <em>this
        ...
        <Section id="hetero" title="Heterogeneity" href="#/hetero">
          <p>
            ...
            rank low (they hit everything uniformly); context-selective targeted agents rank high, and they
            are the substrate of residual disease and resistance.
          </p>
          <p>
            ...
            clusters in more than <N>a third</N> of all drugs. These are the proliferation programmes, where
            cluster differences reflect baseline biology rather than drug selectivity, so discount them.
        ...
        <Section id="rank" title="Rank" href="#/rank">
          <p>
            ...
            sits behind an explicit click with a size warning, a progress readout and a cancel button. Exact
      ...
      <div id="exports" className="mt-4 scroll-mt-24 rounded-lg border border-stone-200 bg-white p-5">
        ...
        <div className="mt-2 space-y-2 text-sm leading-relaxed text-stone-600">
          <p>
            <B>SVG ↓</B> gives a standalone vector figure with the fonts and background inlined, ready to
            drop straight into a manuscript. <B>CSV ↓</B> gives the numbers behind the chart, including the
